...
//static void	ft_double_quotes(t_minishell *sh, t_node *node);
static void	ft_double_quotes(t_minishell *sh, t_node *node, int c);
//static void	ft_single_quotes(t_minishell *sh, t_node *node, int *status);
//static void	ft_single_quotes(t_minishell *sh, t_node *node, int c, int *status);
...
//	printf("Antes de valid quotes com '\''\n");
...
static void	ft_single_quotes(t_minishell *sh, t_node *node, int *status)
{
	printf("Dentro da single_quotes | Início\n");
	long	i;
	long	start;
	long	end;
	long	first;
	...
	printf("Dentro da single_quotes | Início - status: %s\n", node->cmd[1]);
	while (node->cmd[1][i])
	{
		while (node->cmd[1][i] && (node->cmd[1][i] == '\''))
		{
			i++;
			count++;
		}
		printf("Dentro da single_quotes | count: %ld\n", count);
		if (status)
		{
			printf("Dentro da single_quotes | dentro do if status | count: %ld | Início - status: %i\n", count, *status);
			if (count % 2)
			{
				*status = FALSE;
				printf("Dentro da single_quotes | dentro do if count: %ld | Início - status: %i\n", count, *status);
			}
			count = 0;
		}
		start = i;
	...
	printf("Dentro da single_quotes | Fim\n");
}
*/
...
static void	ft_double_quotes(t_minishell *sh, t_node *node, int c)
{
	printf("Dentro da double_quotes | Início\n");
	long	i;
	long	start;
	long	end;
	long	first;
	...
	printf("Dentro da double_quotes | Fim\n");
}
*/
